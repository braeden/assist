...
class ToolRouter @Inject constructor(
    ...
    suspend fun execute(sessionId: Long, call: ToolCall): ToolExecution {
        ...
        return try {
            when (call.name) {
                ...
                AgentTools.LONG_PRESS_XY -> control(call, device.longPressXy(args.int("x"), args.int("y")))
                AgentTools.SWIPE -> swipe(call, args)
                AgentTools.SWIPE_XY -> swipeXy(call, args)
                AgentTools.SCROLL -> scroll(call, args)
                AgentTools.SET_TEXT -> control(call, device.setText(args.int("element_id"), args.str("text")))
                AgentTools.PRESS_KEY -> pressKey(call, args)
                AgentTools.OPEN_APP -> control(call, device.openApp(args.str("app")), settle = true)
    ...
    private suspend fun getScreenState(call: ToolCall): ToolExecution {
        ...
        return ToolExecution(
            resultBlock = textResult(call, state.toOutline()),
            success = true,
    ...
    private suspend fun takeScreenshot(call: ToolCall): ToolExecution {
        val bitmap = device.takeScreenshot()
            ?: return error(call, "Screenshot unavailable (service not connected or rate-limited)")
    ...
    private fun control(call: ToolCall, outcome: ToolOutcome, settle: Boolean = true): ToolExecution =
        ToolExecution(
    ...
    private fun nonActing(call: ToolCall, outcome: ToolOutcome): ToolExecution =
        ToolExecution(
            resultBlock = outcomeResult(call, outcome),
            success = outcome.success,
            message = outcome.message,
    ...
    private suspend fun swipe(call: ToolCall, args: Args): ToolExecution {
        val dir = SwipeDirection.fromString(args.str("direction"))
    ...
    private suspend fun swipeXy(call: ToolCall, args: Args): ToolExecution {
        val duration = args.longOrNull("duration_ms")
        val outcome = if (duration != null) {
            device.swipeXy(args.int("x1"), args.int("y1"), args.int("x2"), args.int("y2"), duration)
        } else {
    ...
    private suspend fun scroll(call: ToolCall, args: Args): ToolExecution {
        val elementId = args.intOrNull("element_id")
        val outcome = if (elementId != null) {
            device.scroll(elementId, args.boolOrNull("forward") ?: true)
        } else {
            val dir = args.strOrNull("direction")?.let { SwipeDirection.fromString(it) }
    ...
    private suspend fun pressKey(call: ToolCall, args: Args): ToolExecution {
        val key = DeviceKey.fromString(args.str("key"))
    ...
    private suspend fun say(call: ToolCall, args: Args): ToolExecution {
        val text = args.str("text")
        userIo.say(text)
        return textResultExec(call, "spoke", "ok")
    }

    private suspend fun ask(call: ToolCall, args: Args): ToolExecution {
    ...
    private suspend fun dropOldScreenshots(sessionId: Long, call: ToolCall, args: Args): ToolExecution {
        ...
        return ToolExecution(
            ...
            contextEdit = ContextManagement(clearToolUses = true, keepLastToolUses = keepLast.takeIf { it > 0 }),
        )
    }

    private fun compact(sessionId: Long, call: ToolCall): ToolExecution =
        ToolExecution(
    ...
    private suspend fun note(sessionId: Long, call: ToolCall, args: Args): ToolExecution {
        val text = args.str("text")
    ...
    private fun outcomeResult(call: ToolCall, outcome: ToolOutcome): ContentBlock.ToolResult =
    ...
    private fun textResult(call: ToolCall, text: String, isError: Boolean = false): ContentBlock.ToolResult =
        ContentBlock.ToolResult(
            toolUseId = call.id,
    ...
    private fun textResultExec(call: ToolCall, message: String, resultText: String) =
        ToolExecution(resultBlock = textResult(call, resultText), success = true, message = message)

    private fun error(call: ToolCall, message: String): ToolExecution =
        ToolExecution(
    ...
    private class Args(
        ...
        fun int(key: String): Int = intOrNull(key) ?: throw IllegalArgumentException("missing int '$key'")
        fun longOrNull(key: String): Long? = obj[key]?.jsonPrimitive?.content?.toLongOrNull()
        fun long(key: String): Long = longOrNull(key) ?: throw IllegalArgumentException("missing '$key'")
        ...
        fun str(key: String): String = strOrNull(key) ?: throw IllegalArgumentException("missing string '$key'")
    }
