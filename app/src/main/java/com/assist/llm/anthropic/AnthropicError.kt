...
class AnthropicAuthException(message: String, errorType: String? = null) :
...
class AnthropicRateLimitException(message: String, retryAfterSeconds: Long? = null, errorType: String? = null) :
    AnthropicException(message, statusCode = 429, errorType = errorType, retryable = true, retryAfterSeconds = retryAfterSeconds)

/** 5xx — transient server error. */
class AnthropicServerException(message: String, statusCode: Int, errorType: String? = null) :
    AnthropicException(message, statusCode = statusCode, errorType = errorType, retryable = true)

/** 529 — service overloaded. */
class AnthropicOverloadedException(message: String, retryAfterSeconds: Long? = null, errorType: String? = null) :
    AnthropicException(message, statusCode = 529, errorType = errorType, retryable = true, retryAfterSeconds = retryAfterSeconds)

/** Transport failure before/while a response was received. */
class AnthropicNetworkException(message: String, cause: Throwable? = null) :
...
internal object AnthropicErrorMapper {
    ...
    ): AnthropicException = when (statusCode) {
