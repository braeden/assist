...
import com.wisp.memory.MemoryPaths
...
class TaskMemoryRepository(
    ...
    suspend fun recipeContent(id: Long): String? =
    ...
    suspend fun recallHint(
        ...
        withContext(ioDispatcher) {
            ...
            recipes
                ...
                .filter { it.second > 0 }
                .sortedWith(
                    compareByDescending<Pair<TaskRecipeEntity, Int>> {
                        it.second
     ...
     */
    suspend fun onMemoryMutation(input: JsonObject) =
        withContext(ioDispatcher) {
            when (input.str("command")) {
                "view" ->
                    input
                        .str(
                            "path",
                        )?.let { if (MemoryPaths.isTaskRecipe(it)) recipes.recordUse(it, now()) }
                ...
                "delete" ->
                    input
                        .str(
                            "path",
                        )?.let { if (MemoryPaths.isTaskRecipe(it)) recipes.deleteByPath(it) }
                "rename" -> {
    ...
    private fun score(
        wanted: Set<String>,
        have: Set<String>,
    ): Int = wanted.count { it in have }

    private fun TaskRecipeEntity.toDomain() =
        TaskRecipe(
            id = id,
            title = title,
            appPackage = appPackage,
