...
class AgentService : Service() {
    ...
    override fun onCreate() {
        super.onCreate()
        ensureChannel()
        startForegroundInternal("Wisp agent ready")
    }
    ...
    private fun ensureChannel() {
        ...
        if (manager.getNotificationChannel(CHANNEL_ID) == null) {
            manager.createNotificationChannel(
                NotificationChannel(CHANNEL_ID, "Wisp agent", NotificationManager.IMPORTANCE_LOW),
            )
        }
    }
    ...
    companion object {
        ...
        const val ACTION_RUN = "com.wisp.action.AGENT_RUN"
        const val ACTION_INTERRUPT = "com.wisp.action.AGENT_INTERRUPT"
