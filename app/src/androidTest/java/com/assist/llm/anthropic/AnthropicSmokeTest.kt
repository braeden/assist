...
class AnthropicSmokeTest {
    ...
    fun textTurn() = runBlocking {
        ...
        val response = client.send(request) { event ->
    ...
    @Test
    fun toolUseTurn() = runBlocking {
        val echoTool = ToolDef(
            name = "echo",
        ...
        val request = baseRequest(
            ...
            messages = listOf(
                userText("Use the echo tool to echo the word banana. Do not reply otherwise."),
            ),
        )
    ...
    @Test
    fun visionTurn() = runBlocking {
        val request = baseRequest(
            messages = listOf(
                LlmMessage(
                    role = Role.USER,
                    content = listOf(
                        ContentBlock.Image(base64 = redPngBase64(), mediaType = "image/png"),
                        ContentBlock.Text("What color is this image? Answer with a single word."),
                    ),
                ),
            ),
        )
