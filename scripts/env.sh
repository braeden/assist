...
_wisp_self="${BASH_SOURCE[0]:-$0}"
WISP_ROOT="$(cd "$(dirname "$_wisp_self")/.." && pwd)"

# --- Android SDK ----------------------------------------------------------
# Honor ANDROID_HOME / ANDROID_SDK_ROOT; otherwise use the OS default location.
...
APP_ID="com.wisp"
APP_MODULE=":app"
LAUNCHER_ACTIVITY="com.wisp/.ui.MainActivity"   # created in phase-02
DEBUG_APK="$WISP_ROOT/app/build/outputs/apk/debug/app-debug.apk"
AVD_NAME="pixel7pro_api35"
GRADLEW="$WISP_ROOT/gradlew"

# --- Helpers --------------------------------------------------------------
# Generate the (gitignored, machine-specific) local.properties from the resolved
# SDK path if it's missing, so a fresh clone builds without manual setup.
ensure_local_properties() {
  local lp="$WISP_ROOT/local.properties"
  if [[ ! -f "$lp" ]]; then
    printf '## Auto-generated by scripts/env.sh. Gitignored; do not commit.\nsdk.dir=%s\n' \
      "$ANDROID_HOME" > "$lp"
    echo ">> wrote $lp (sdk.dir=$ANDROID_HOME)" >&2
  fi
}
